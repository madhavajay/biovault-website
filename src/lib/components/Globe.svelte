<script lang="ts">
	...
	onMount(() => {
		...
		// Scale camera distance based on viewport - closer on larger screens, further on small
		const getCameraZ = () => {
			if (sizes.width < 640) return 600; // Pull camera back on mobile so globe is narrower
			return Math.max(350, Math.min(500, 600 - sizes.width * 0.15));
		};
		...
		const pointLight = new THREE.PointLight(0x22c55e, 12, 200);
		pointLight.position.set(-50, 0, 60);
		scene.add(pointLight);
		scene.add(new THREE.HemisphereLight(0xffffff, 0xd4edda, 1.5));
		...
		const boot = async () => {
			...
			if (globeData?.features) {
				globe
					...
					.polygonCapColor(() => "rgba(34, 197, 94, 0.5)")
					.polygonSideColor(() => "rgba(16, 185, 129, 0.15)")
					.polygonStrokeColor(() => "rgba(20, 184, 166, 0.4)")
